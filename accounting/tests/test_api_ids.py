...
class AccountingApiIdHardeningTest(TestCase):
        ...
        for value in (0, -1, 9223372036854775808, '9' * 100):
            with self.subTest(consumer='custodian', value=value):
                response = self.client.post(
                    ...
                    HTTP_IDEMPOTENCY_KEY=f'custodian-{value}',
                )
            ...
            with self.subTest(consumer='account', value=value):
                response = self.client.post(
                    ...
                    content_type='application/json',
                    HTTP_IDEMPOTENCY_KEY=f'account-{value}',
                )
                self.assertEqual(response.status_code, 400)
    ...
    def test_very_long_numeric_ids_are_api_input_errors_before_integer_conversion(self):
        value = '9' * 5000
        responses = [
            self.client.post(
                '/api/accounting/accounts/',
                {'name': 'long id', 'currency': 'CNY', 'custodian_id': value},
                content_type='application/json',
                HTTP_IDEMPOTENCY_KEY='long-custodian',
            ),
            self.client.post(
                '/api/accounting/opening-balances/',
                {
                ...
                content_type='application/json',
                HTTP_IDEMPOTENCY_KEY='long-account',
            ),
            self.client.get('/api/accounting/transactions/', {'account_id': value}),
        ]
