...
describe('账务动作中心 SSR 契约', () => {
  it('四张动作卡使用可聚焦的稳定引导 section', () => {
    const html = renderToStaticMarkup(<AccountingActionCenter {...({ accounts: [], purchases: [], dividends: [], businessDate: '2026-08-15', onChanged: () => undefined } as never)} />);
  ...
  it('摘要账户应优先使用带余额的 summaryAccounts 数据', () => {
    const html = renderToStaticMarkup(<AccountingActionCenter {...({
      ...
      summaryAccounts: [{ id: 1, name: '人民币账户', currency: 'CNY', is_active: true, original_balance: '12345.67', cny_book_cost: '12345.67' }],
      purchases: [],
      dividends: [],
