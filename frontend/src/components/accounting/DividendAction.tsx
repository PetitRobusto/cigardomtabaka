...
function DividendActionForm({
  ...
  return (
    <section tabIndex={-1} data-guide="accounting-actions-dividend" className="rounded-2xl border border-[#E8E0D6] bg-[#FAF8F5] p-5 text-[#2C2416]">
      ...
      {!draft ? (
        <div className="grid gap-3 sm:grid-cols-[1fr_1fr_auto] sm:items-end"><label className="text-sm">分红总额<input data-guide="accounting-dividend-total" inputMode="decimal" value={input.total} onChange={event => patchInput({ total: event.target.value })} placeholder="0.00" className="mt-1 block w-full rounded-lg border border-[#E8E0D6] p-2" /></label><label className="text-sm">业务日期<input data-guide="accounting-dividend-date" type="date" value={input.businessDate} onChange={event => patchInput({ businessDate: event.target.value })} className="mt-1 block w-full rounded-lg border border-[#E8E0D6] p-2" /></label><span data-guide="accounting-dividend-create"><button type="button" disabled={busy || !input.total || !input.businessDate} onClick={create} className="rounded-lg bg-[#7A1F2E] px-3 py-2 text-sm text-white disabled:opacity-50">{busy ? '处理中…' : '创建分红草稿'}</button></span></div>
      ) : (
        <div className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-3"><label className="text-sm">总额<input data-guide="accounting-dividend-total" inputMode="decimal" value={input.total} onChange={event => patchInput({ total: event.target.value })} className="mt-1 block w-full rounded-lg border border-[#E8E0D6] p-2" /></label><label className="text-sm">合伙人 A<input data-guide="accounting-dividend-partner-a" inputMode="decimal" value={input.partnerA} onChange={event => patchInput({ partnerA: event.target.value })} className="mt-1 block w-full rounded-lg border border-[#E8E0D6] p-2" /></label><label className="text-sm">合伙人 B<input data-guide="accounting-dividend-partner-b" inputMode="decimal" value={input.partnerB} onChange={event => patchInput({ partnerB: event.target.value })} className="mt-1 block w-full rounded-lg border border-[#E8E0D6] p-2" /></label></div>
          <div className="grid gap-3 sm:grid-cols-2"><label className="text-sm">合伙人 A 人民币账户<select data-guide="accounting-dividend-account-a" value={input.accountA} onChange={event => patchInput({ accountA: event.target.value })} className="mt-1 block w-full rounded-lg border border-[#E8E0D6] bg-white p-2"><option value="">选择账户</option>{accountOptionsA.map(({ account, disabled }) => <option key={account.id} value={account.id} disabled={disabled}>{account.name}</option>)}</select></label><label className="text-sm">合伙人 B 人民币账户<select data-guide="accounting-dividend-account-b" value={input.accountB} onChange={event => patchInput({ accountB: event.target.value })} className="mt-1 block w-full rounded-lg border border-[#E8E0D6] bg-white p-2"><option value="">选择账户</option>{accountOptionsB.map(({ account, disabled }) => <option key={account.id} value={account.id} disabled={disabled}>{account.name}</option>)}</select></label></div>
          ...
          <div className="flex flex-wrap gap-2"><span data-guide="accounting-dividend-save"><button type="button" disabled={busy || !canSave} onClick={update} className="rounded-lg border border-[#7A1F2E] px-3 py-2 text-sm text-[#7A1F2E] disabled:opacity-50">{busy ? '处理中…' : '保存分红编辑'}</button></span><span data-guide="accounting-dividend-preview"><button type="button" disabled={busy || !previewValidation.ok} onClick={previewAction} className="rounded-lg bg-[#7A1F2E] px-3 py-2 text-sm text-white disabled:opacity-50">预览</button></span></div>
          {preview && previewIsCurrent && <div data-guide="accounting-dividend-preview-result" className="rounded-xl border border-[#E8E0D6] bg-white p-3 text-sm"><div className="grid gap-2 sm:grid-cols-3"><span>未分配利润：{preview.retained_earnings_cny || '—'} 元</span><span>申请分红：{preview.requested_cny || '—'} 元</span><span>提示指纹：{preview.warning_fingerprint || '—'}</span></div>{warning && <label className="mt-3 flex items-start gap-2 text-[#7A1F2E]"><input data-guide="accounting-dividend-warning" type="checkbox" checked={acknowledged} onChange={event => setAcknowledged(event.target.checked)} />我已确认：{warning}</label>}<span data-guide="accounting-dividend-confirm" className="mt-3 block w-fit"><button type="button" disabled={busy || draftDirty || !previewIsCurrent || Boolean(warning && !acknowledged)} onClick={confirm} className="rounded-lg bg-[#7A1F2E] px-3 py-2 text-sm text-white disabled:opacity-50">确认分红</button></span></div>}
        </div>
      )}
