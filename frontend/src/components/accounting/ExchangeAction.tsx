    ...
    <section tabIndex={-1} data-guide="accounting-actions-exchange" className="rounded-md border border-border bg-white shadow-sm">
      ...
      <form onSubmit={submitForm} className="space-y-3 p-5">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <label className="block text-xs font-medium text-muted">转出账户<select data-guide="accounting-exchange-source-account" required value={sourceId} onChange={event => update({ source_account_id: Number(event.target.value) })} className="mt-1.5 w-full rounded border border-border px-3 py-2 text-sm"><option value="" disabled>选择 CNY 或 USDT 账户</option>{sources.map(account => <option key={account.id} value={account.id}>{account.name} · {account.currency}</option>)}</select></label>
          <label className="block text-xs font-medium text-muted">转入账户<select data-guide="accounting-exchange-rub-account" required value={rubId} onChange={event => update({ rub_account_id: Number(event.target.value) })} className="mt-1.5 w-full rounded border border-border px-3 py-2 text-sm"><option value="" disabled>选择 RUB 账户</option>{rubAccounts.map(account => <option key={account.id} value={account.id}>{account.name} · RUB</option>)}</select></label>
          <label className="block text-xs font-medium text-muted">实际转出数量<input data-guide="accounting-exchange-source-amount" required value={current.source_amount} onChange={event => update({ source_amount: event.target.value })} inputMode="decimal" placeholder="例如 1000.00" className="mt-1.5 w-full rounded border border-border px-3 py-2 text-sm" /></label>
          <label className="block text-xs font-medium text-muted">实际转入数量（RUB）<input data-guide="accounting-exchange-rub-amount" required value={current.rub_amount} onChange={event => update({ rub_amount: event.target.value })} inputMode="decimal" placeholder="例如 12000.00" className="mt-1.5 w-full rounded border border-border px-3 py-2 text-sm" /></label>
        </div>
