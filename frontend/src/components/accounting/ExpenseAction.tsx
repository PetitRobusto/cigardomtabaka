  ...
  return (
    <section tabIndex={-1} data-guide="accounting-actions-expense" className="rounded-md border border-border bg-white shadow-sm">
      ...
      <form onSubmit={submitForm} className="space-y-3 p-5">
        ...
        <label className="block text-xs font-medium text-muted">费用类别<select data-guide="accounting-expense-category" value={normaliseCategory(currentCategory)} onChange={event => updateCategory(event.target.value as ExpenseCategory)} className="mt-1.5 w-full rounded border border-border px-3 py-2 text-sm">{Object.entries(categoryLabel).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label>
        <label className="block text-xs font-medium text-muted">资金账户（{currency}）<select data-guide="accounting-expense-account" required value={accountId} onChange={event => update({ fund_account_id: Number(event.target.value) })} className="mt-1.5 w-full rounded border border-border px-3 py-2 text-sm"><option value="" disabled>选择 {currency} 账户</option>{filteredAccounts.map(account => <option key={account.id} value={account.id}>{account.name} · {account.currency}</option>)}</select></label>
