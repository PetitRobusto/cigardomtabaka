...
import { declaredBoxSizes, inventoryLineTotal, isLatestDay1SearchRequest } from '../../features/day1/day1State';
...
export default function Day1InventoryStep({ inventory, onChange, fieldErrors = {}, declaredBoxSizesByCigar, readOnly = false }: Props) {
  ...
  const add = (cigar: SearchCigarResult) => {
    ...
    fetchCigarDetail(cigar.id).then(detail => {
      const boxSizes = boxSizesFromCigarDetail(detail);
      const boxSize = boxSizes[0] || 0;
      setLoadedBoxSizes(current => ({ ...current, [cigar.id]: boxSizes }));
