...
import { inventoryLineTotal, isLatestDay1SearchRequest, uniqueDay1InventoryCigarIds } from '../../features/day1/day1State';
...
export default function Day1InventoryStep({ inventory, onChange, fieldErrors = {}, declaredBoxSizesByCigar, readOnly = false }: Props) {
  ...
  return <section className="rounded-md border border-border bg-white p-5 shadow-sm">
    ...
    {!readOnly && <div className="relative mb-5"><label className="text-xs font-medium text-muted">搜索目录雪茄<input disabled={adding} value={query} onChange={event => setQuery(event.target.value)} placeholder="输入中文名或英文名" className="mt-1 w-full rounded border border-border px-3 py-2 text-sm text-fg" /></label>{visibleResults.length > 0 && <div className="absolute left-0 right-0 top-full z-10 mt-1 overflow-hidden rounded border border-border bg-white shadow-lg">{visibleResults.slice(0, 8).map(cigar => <button key={cigar.id} type="button" disabled={adding} onClick={() => add(cigar)} className="flex w-full items-center justify-between border-b border-border px-3 py-2 text-left text-sm last:border-0 hover:bg-cream disabled:opacity-50"><span><strong>{cigar.name}</strong><small className="ml-2 text-muted">{cigar.english_name}</small></span><span className="text-xs text-muted">{cigar.batches.length} 批次</span></button>)}</div>}</div>}
