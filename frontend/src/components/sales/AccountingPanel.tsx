...
{showStats && <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
    ...
    <section className={showProfit ? 'grid gap-5 lg:grid-cols-[1fr_1.15fr]' : ''}>
      ...
      {showProfit && <div data-guide="accounting-profit" className="rounded-md border border-border bg-white shadow-sm"><div className="flex items-center justify-between border-b border-border px-5 py-4"><div><h2 className="font-display text-lg font-semibold">月度利润</h2><p className="text-xs text-muted">商品成本和人肉成本均来自已入账流水</p></div><span className="font-mono text-xs text-muted">{profit?.transaction_count ?? '—'} {profit ? '笔流水' : ''}</span></div>{profitError ? <RegionError message={profitError} /> : !profit ? <RegionLoading /> : <div className="grid grid-cols-2 gap-3 p-5"><Metric label="销售收入" value={incomeMoney(Number(profit.sales_revenue_cny) + Number(profit.customer_transport_revenue_cny))} /><Metric label="商品成本" value={expenseMoney(profit.cost_of_goods_sold_cny)} /><Metric label="人肉成本" value={formatSignedCny(-(Math.abs(Number(profit.transport_expense_cny)) + Math.abs(Number(profit.transport_settlement_expense_cny || 0))))} /><div className="col-span-2 rounded border border-green-100 bg-green-50 p-3"><p className="text-[11px] text-muted">净利润</p><p className="mt-1 font-mono text-xl font-semibold text-success">{formatSignedCny(profit.net_profit_cny)}</p></div></div>}</div>}
    </section>
