...
export default function AccountingPanel({ accounts, summary, profit, reconciliations, month, onChanged }: Props) {
  ...
  const submitRecon = async (event: React.FormEvent) => {
    ...
    if (!accountId || !validateMoneyInput(actual)) { setError('请选择可用账户并输入非负、最多两位小数的实际余额'); setBusy(false); return; }
    try { await createReconciliation({ account_id: accountId, business_date: date, actual_amount: actual, note }); setShowRecon(false); setActual(''); setNote(''); onChanged(); }
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-md border border-border bg-white shadow-sm"><div className="flex items-center justify-between border-b border-border px-5 py-4"><div><h2 className="font-display text-lg font-semibold">账户对账</h2><p className="text-xs text-muted">创建实盘快照后，确认才会完成对账闭环。</p></div><ClipboardCheck className="h-5 w-5 text-gold" /></div>{error && <p className="mx-5 mt-3 rounded bg-red-50 px-3 py-2 text-xs text-red-700">{error}</p>}<div className="divide-y divide-border">{reconciliations.length === 0 ? <p className="px-5 py-8 text-center text-sm text-muted">暂无对账记录</p> : reconciliations.map(item => { const account = accounts.find(value => value.id === item.account_id); return <div key={item.id} className="flex flex-col gap-2 px-5 py-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold">{account?.name || `账户 #${item.account_id}`} <span className="ml-2 text-xs font-normal text-muted">{item.business_date}</span></p><p className="text-xs text-muted">系统 {item.system_amount} · 实盘 {item.actual_amount} · 差异 <span className={Number(item.difference) === 0 ? 'text-success' : 'text-accent'}>{item.difference}</span></p></div>{item.status === 'pending' ? <button type="button" disabled={busy} onClick={() => confirm(item.id)} className="inline-flex w-fit items-center gap-1 rounded bg-accent px-3 py-1.5 text-xs font-semibold text-white disabled:opacity-50"><Check className="h-3.5 w-3.5" />确认对账</button> : <span className="rounded-full bg-green-50 px-2 py-1 text-[10px] font-bold text-success">已确认</span>}</div>; })}</div></section>
    {showRecon && <div role="dialog" aria-modal="true" aria-labelledby="reconciliation-title" className="fixed inset-0 z-50 grid place-items-center bg-fg/30 p-4"><form onSubmit={submitRecon} className="w-full max-w-md rounded-md border border-border bg-white shadow-lg"><div className="flex items-center justify-between border-b border-border px-5 py-4"><h2 id="reconciliation-title" className="font-display text-lg font-semibold">新建账户对账</h2><button type="button" onClick={() => setShowRecon(false)} className="text-muted">×</button></div><div className="space-y-3 p-5"><label className="block text-xs font-medium text-muted">资金账户<select required value={accountId} onChange={event => setAccountId(Number(event.target.value))} className="mt-1.5 w-full rounded border border-border px-3 py-2 text-sm">{activeAccounts.map(account => <option key={account.id} value={account.id}>{account.name} · {account.currency}</option>)}</select></label><label className="block text-xs font-medium text-muted">业务日期<input required type="date" value={date} onChange={event => setDate(event.target.value)} className="mt-1.5 w-full rounded border border-border px-3 py-2 text-sm" /></label><label className="block text-xs font-medium text-muted">实际余额<input required value={actual} onChange={event => setActual(event.target.value)} inputMode="decimal" className="mt-1.5 w-full rounded border border-border px-3 py-2 text-sm" /></label><label className="block text-xs font-medium text-muted">备注<textarea value={note} onChange={event => setNote(event.target.value)} rows={2} className="mt-1.5 w-full rounded border border-border px-3 py-2 text-sm" /></label>{error && <p className="text-xs text-red-700">{error}</p>}<button disabled={busy || !accountId || !actual} className="w-full rounded bg-accent px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{busy ? '创建中…' : '创建对账'}</button></div></form></div>}
  </div>;
}
