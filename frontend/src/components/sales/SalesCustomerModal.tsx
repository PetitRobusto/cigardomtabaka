  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      const saved = mode === 'create'
        ? await createSalesCustomer({ name: name.trim(), phone: phone.trim(), remark: remark.trim() })
        : await updateSalesCustomer(customerId!, { name: name.trim(), phone: phone.trim(), remark: remark.trim() });
  ...
  return <div role="dialog" aria-modal="true" aria-label={mode === 'create' ? '新建客户' : '客户资料'} className="fixed inset-0 z-50 grid place-items-center bg-fg/30 p-4" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
    <div className="max-h-[calc(100vh-2rem)] w-full max-w-xl overflow-y-auto rounded-md border border-border bg-white shadow-xl">
      ...
      {(mode === 'create' || editing) && <form onSubmit={submit} className="space-y-4 p-5">
        ...
        <label className="block text-xs font-medium text-muted">备注<textarea value={remark} onChange={event => setRemark(event.target.value)} maxLength={2000} rows={3} placeholder="客户偏好、交付约定等" className="mt-1.5 w-full resize-none rounded border border-border px-3 py-2 text-sm text-fg outline-none focus:border-gold" /></label>
      ...
      {mode === 'detail' && !editing && <>
        ...
        {customer && <div className="space-y-5 p-5">
          <div className="rounded border border-border bg-[#FFFDFA] p-3 text-xs"><span className="text-muted">备注</span><p className="mt-1 whitespace-pre-wrap font-medium">{customer.remark || '—'}</p></div>
          ...
          <div><div className="mb-2 flex items-center justify-between"><h3 className="text-xs font-bold uppercase tracking-wide">最近订单</h3><button type="button" onClick={() => { setName(customer.name); setPhone(customer.phone); setRemark(customer.remark || ''); setEditing(true); }} className="text-xs font-semibold text-accent">编辑资料</button></div>
