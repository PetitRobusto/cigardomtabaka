...
import { cigarSearchParams, formatCny, transportPayerChange, transportSummary, validateMoneyInput, type TransportPayer } from './salesState';
    ...
    <form data-guide="sales-orders" onSubmit={submit} className="rounded-md border border-border bg-white shadow-sm">
      ...
      <div className="space-y-4 p-5">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <label className="text-xs font-medium text-muted">客户名称<input data-guide="sales-order-customer" value={customerName} onChange={event => setCustomerName(event.target.value)} placeholder="散客或客户姓名" className="mt-1.5 w-full rounded border border-border px-3 py-2 text-sm outline-none focus:border-gold" /></label>
