    ...
    const orders = [
      {
        ...
        payment_status: "unpaid",
        amount_due_cny: 100,
        contribution_profit: 30,
      },
      {
        fulfillment_status: "confirmed",
        payment_status: "unpaid",
        amount_due_cny: 200,
        contribution_profit: 60,
      },
      {
        fulfillment_status: "shipped",
        payment_status: "paid",
        amount_due_cny: 300,
        contribution_profit: 90,
      },
      {
        fulfillment_status: "returned",
        payment_status: "refunded",
        amount_due_cny: 400,
        contribution_profit: 120,
      },
    ];
...
describe("sales business state", () => {
  it("计算人肉费承担方对应的客户费用和应收", async () => {
    ...
    expect(transportSummary("customer", 1000, 80)).toEqual({
      goodsAmount: 1000,
      customerTransport: 80,
      amountDue: 1080,
    });
    expect(transportSummary("company", 1000, 80)).toEqual({
      goodsAmount: 1000,
      customerTransport: 0,
      amountDue: 1000,
    });
  ...
  it("focus 搜索使用空 query 且只搜现货", async () => {
    ...
    expect(cigarSearchParams("input", "Partagas")).toEqual({
      q: "Partagas",
      stock_only: true,
    });
  });
