...
const salesSteps: readonly ContextTourStep[] = [
  ...
  { id: 'accounting-reconciliation', title: '账户对账', description: '用实际余额核对系统账面，确认每一笔差异。', target: '[data-guide="accounting-reconciliation"]', route: '/sales#accounting' },
  { id: 'accounting-profit', title: '查看月利润', description: '月度利润汇总收入、批次成本与费用。', target: '[data-guide="accounting-profit"]', route: '/sales#accounting' },
];
...
export function tourStepsForRoute(route: string, id?: string): readonly ContextTourStep[] {
  ...
  const group = pathname === '/' ? 'overview' : pathname === '/sales' ? (route.includes('#accounting') ? 'accounting' : 'sales') : pathname === '/inventory' ? 'inventory' : pathname === '/privnote' ? 'privnote' : pathname === '/prices' ? 'prices' : '';
