...
export interface Day1BackgroundButtonProps {
  testId: string;
  dialogOpen: boolean;
  onClick: () => void;
  children: ReactNode;
...
export function Day1BackgroundButton({ testId, dialogOpen, onClick, children, disabled = false, className = '', ariaCurrent }: Day1BackgroundButtonProps) {
  // Centralize the aria-modal mouse barrier so a new background button cannot omit the guard.
  return <button type="button" data-testid={`day1-background-${testId}`} aria-current={ariaCurrent} disabled={disabled || day1BackgroundInteractionDisabled(dialogOpen)} onClick={() => runDay1BackgroundAction(dialogOpen, onClick)} className={className}>{children}</button>;
}
...
export default function Day1SetupPage() {
  ...
  // aria-modal does not freeze mouse events, so disable every wizard background action explicitly.
  const backgroundDisabled = day1BackgroundInteractionDisabled(confirmationOpen);
  const versionMismatch = Boolean(server && server.status !== 'completed' && draftBaseVersion !== server.version);
  return <div className="w-full animate-fade-in"><header className="mb-6 flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><p className="text-[11px] font-bold uppercase tracking-[.12em] text-accent">Opening setup · Day 1</p><h1 className="mt-1 font-display text-3xl font-semibold tracking-tight sm:text-4xl">一次性初始化</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted">共同整理期初账户和库存。确认前不会影响账务；确认后一次性生效且不可重跑。</p></div><div className="flex flex-wrap items-center justify-end gap-2 text-xs text-muted"><span>本地基准 v{draftBaseVersion} · 共享 v{server?.version || 0}</span><Day1BackgroundButton testId="refresh" dialogOpen={confirmationOpen} disabled={backgroundDisabled} onClick={() => load(true)} className="rounded border border-border bg-white px-3 py-2 hover:border-gold disabled:opacity-40">刷新共享状态</Day1BackgroundButton><Day1BackgroundButton testId="discard" dialogOpen={confirmationOpen} disabled={backgroundDisabled} onClick={() => load(false)} className="rounded border border-border bg-white px-3 py-2 text-accent hover:border-gold disabled:opacity-40">放弃本地并加载共享草稿</Day1BackgroundButton></div></header>
    ...
    <div className="grid gap-5 lg:grid-cols-[220px_1fr]"><aside className="rounded-md border border-border bg-white p-2 shadow-sm"><div className="hidden lg:block">{stepLabels.map((label, index) => <Day1BackgroundButton testId={`step-desktop-${index + 1}`} dialogOpen={confirmationOpen} key={label} ariaCurrent={step === index + 1 ? 'step' : undefined} disabled={backgroundDisabled} onClick={() => setStepSafe(index + 1)} className={`flex w-full items-center gap-3 rounded px-3 py-3 text-left text-sm ${step === index + 1 ? 'bg-accent-light text-accent font-semibold' : 'text-muted hover:bg-cream'}`}><span className={`grid h-7 w-7 place-items-center rounded-full text-xs font-bold ${step > index + 1 ? 'bg-green-600 text-white' : 'bg-cream'}`}>{index + 1}</span>{label}</Day1BackgroundButton>)}</div><div className="flex justify-between gap-1 lg:hidden">{stepLabels.map((label, index) => <Day1BackgroundButton testId={`step-mobile-${index + 1}`} dialogOpen={confirmationOpen} key={label} ariaCurrent={step === index + 1 ? 'step' : undefined} disabled={backgroundDisabled} onClick={() => setStepSafe(index + 1)} className={`flex flex-1 flex-col items-center gap-1 rounded px-1 py-2 text-center text-[11px] ${step === index + 1 ? 'bg-accent-light text-accent font-semibold' : 'text-muted'}`}><span className="grid h-6 w-6 place-items-center rounded-full bg-cream text-xs">{index + 1}</span>{label}</Day1BackgroundButton>)}</div></aside>
      <main className="min-w-0">{step === 1 && <RulesStep date={draft.business_date} readOnly={false} businessDateError={validationDetails.business_date} onDateChange={businessDate => { clearValidationDetails('business_date'); setDraft(current => ({ ...current, business_date: businessDate })); }} />}{step === 2 && <Day1AccountsStep accounts={draft.accounts} fieldErrors={validationDetails} onChange={accounts => { clearValidationDetails('accounts'); setDraft(current => ({ ...current, accounts })); }} />}{step === 3 && <Day1InventoryStep inventory={draft.inventory} fieldErrors={validationDetails} onChange={inventory => { clearValidationDetails('inventory'); setDraft(current => ({ ...current, inventory })); }} />}{step === 4 && <Day1ReviewStep draft={draft} errors={errors} confirmationOpen={confirmationOpen} acknowledged={acknowledged} confirming={confirming} prepareButtonRef={prepareButtonRef} onPrepare={prepareConfirm} onAcknowledge={setAcknowledged} onCancel={cancelConfirm} onConfirm={confirm} />}<div className="mt-4 flex flex-wrap justify-between gap-2"><Day1BackgroundButton testId="previous" dialogOpen={confirmationOpen} disabled={backgroundDisabled || step === 1 || confirming} onClick={() => setStepSafe(previousDay1Step(step))} className="rounded border border-border bg-white px-4 py-2 text-sm disabled:opacity-40">上一步</Day1BackgroundButton><div className="flex gap-2">{!confirmationOpen && <Day1BackgroundButton testId="save" dialogOpen={confirmationOpen} disabled={backgroundDisabled || saving || confirming} onClick={save} className="rounded border border-border bg-white px-4 py-2 text-sm hover:border-gold disabled:opacity-40">{saving ? '保存中…' : '保存草稿'}</Day1BackgroundButton>}{step < day1StepTotal && <Day1BackgroundButton testId="next" dialogOpen={confirmationOpen} disabled={backgroundDisabled || confirming} onClick={() => setStepSafe(nextDay1Step(step))} className="rounded bg-accent px-4 py-2 text-sm font-semibold text-white disabled:opacity-40">下一步</Day1BackgroundButton>}</div></div></main></div>
  </div>;
}
