  ...
  const onFileSelect = useCallback(async (files: FileList | null) => {
    ...
    for (const file of Array.from(files)) {
      try {
        ...
        if (res.url)
          newImages.push({ url: res.url, name: res.name || file.name });
      } catch {
      ...
      {images.length > 0 && (
        <div className="flex gap-2 flex-wrap mt-3">
          {images.map((img, idx) => (
            <div
              key={idx}
              className="relative group inline-flex items-center gap-1.5 px-3 py-1.5 bg-accent-light rounded-full text-xs"
            >
...
export default function PrivnotePage() {
  ...
  const selectedPaymentOrder: PaymentOrder | null =
    eligibleOrders.find(
      (order) => String(order.id) === selectedPaymentOrderId,
    ) || null;
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "私密链接创建失败，请稍后重试",
      );
    } finally {
      setLoading(false);
    }
  ...
  const canSubmit = () => {
    ...
    if (activeTab === "payment")
      return canSubmitPayment(selectedPaymentOrder, paymentMethodId);
    if (activeTab === "message")
      return (
        !!messageText.trim() ||
        attachments.length > 0 ||
        messageImages.images.length > 0
      );
    if (activeTab === "quote") {
  ...
  const actionHint = () => {
    ...
    if (activeTab === "quote") {
      ...
      return visibleSelectedIds.length > 0
        ? `已选 ${visibleSelectedIds.length} 项商品`
        : "请勾选要包含在报价单中的雪茄";
    }
    if (activeTab === "payment")
      return selectedPaymentOrder
        ? "已选择销售单，可创建收款单"
        : "选择待收款销售单和收款方式，创建付款链接";
    return "输入消息内容并上传附件";
  };
    ...
    >
      ...
      <div className="mb-5 inline-flex max-w-full items-center gap-1 rounded-lg border border-border bg-white p-1 shadow-sm">
        <button
          ...
          className={`rounded-md px-4 py-2 text-xs font-semibold transition-colors ${pageMode === "create" ? "bg-accent text-white" : "text-muted hover:text-fg"}`}
        >
          创建链接
        </button>
        <button
          type="button"
          onClick={() => setPageMode("methods")}
          className={`rounded-md px-4 py-2 text-xs font-semibold transition-colors ${pageMode === "methods" ? "bg-accent text-white" : "text-muted hover:text-fg"}`}
        >
          收款方式
        </button>
      </div>
      ...
      {pageMode === "methods" ? (
        ...
        <div className="bg-white border border-border rounded-sm p-6 text-center">
          ...
          <div className="flex items-center gap-2 bg-accent-light rounded-sm p-3 mb-4">
            <code className="text-sm text-fg flex-1 break-all">
              {result.url}
            </code>
            <button
              onClick={copyUrl}
          ...
          <div className="bg-white border border-border rounded-sm p-5 flex items-center gap-8 flex-wrap">
            ...
            <div className="flex items-center gap-3">
              ...
              <select
                ...
                disabled={burn}
                className="px-3 py-2 border border-border rounded-sm text-sm text-fg bg-white disabled:bg-accent-light disabled:text-muted"
              >
            ...
            <div className="flex items-center gap-3 ml-auto">
              <label className="flex items-center gap-3 cursor-pointer">
                <div className="relative">
                  ...
                  />
                  <span
                    className={`block w-10 h-[22px] rounded-full transition-colors ${burn ? "bg-accent" : "bg-border"}`}
                  >
                    <span
                      className={`absolute top-[2px] left-[2px] w-[18px] h-[18px] bg-white rounded-full shadow transition-transform ${burn ? "translate-x-[18px]" : "translate-x-0"}`}
                    />
                  </span>
                </div>
                <div>
            ...
            {activeTab === "inventory" && (
              <div className="bg-white border border-border rounded-sm p-6">
                ...
                <p className="mt-2 text-sm leading-relaxed text-muted">
                  生成当前库存快照，客户只能查看展示内容，不会创建订单、预留库存或改变库存数量。
                </p>
              </div>
            )}
                      ...
                      <button
                        ...
                        onClick={() => {
                          setSelectedIds((current) =>
                            selectedIdsOnCustomEntry(quoteMode, current),
                          );
                          setQuoteMode("custom");
                        }}
                        className={`px-4 py-2 text-xs font-medium rounded-sm transition-all ${
                    ...
                    <div className="flex items-center gap-3 pb-4 border-b border-border">
                      <label className="flex items-center gap-3 cursor-pointer">
                        <div className="relative">
                          ...
                          />
                          <span
                            className={`block w-10 h-[22px] rounded-full transition-colors ${shippingIncluded ? "bg-accent" : "bg-border"}`}
                          >
                            <span
                              className={`absolute top-[2px] left-[2px] w-[18px] h-[18px] bg-white rounded-full shadow transition-transform ${shippingIncluded ? "translate-x-[18px]" : "translate-x-0"}`}
                            />
                          </span>
                        </div>
                        <div>
                      ...
                      <div className="space-y-3">
                        ...
                        {quoteProducts && quoteProducts.length > 0 && (
                          <div className="border border-border rounded-sm bg-white max-h-[480px] overflow-y-auto">
                            {(() => {
                              const filtered = (
                                quoteSearchQ.trim()
                                  ? quoteProducts.filter(
                                      (p) =>
                                        p.brand
                                          .toLowerCase()
                                          .includes(
                                            quoteSearchQ.toLowerCase(),
                                          ) ||
                                        p.name
                                          .toLowerCase()
                                          .includes(
                                        ...
                                        p.english_name
                                          .toLowerCase()
                                          .includes(quoteSearchQ.toLowerCase()),
                                    )
                                  : quoteProducts
                              ).filter((p) => p.can_preorder || p.in_stock);
                              ...
                              return Object.entries(byBrand).map(
                                ([brand, items]) => (
                                  <div
                                    key={brand}
                                    ...
                                    {items.map((p) => (
                                      ...
                                      >
                                        <input
                                          ...
                                          checked={visibleSelectedIds.includes(
                                            p.cigar_id,
                                          )}
                                          onChange={(e) => {
                                            if (e.target.checked) {
                                              setSelectedIds((prev) => [
                                                ...(prev ?? defaultSelectedIds),
                                                p.cigar_id,
                                              ]);
                                            } else {
                                              setSelectedIds((prev) =>
                                        ...
                                        <div className="flex-1 min-w-0">
                                          ...
                                          <div className="text-xs text-muted font-display italic">
                                            {p.english_name}
                                          </div>
                                        </div>
                        ...
                        <div className="flex items-center justify-between pt-3 border-t border-border">
                          <span className="text-sm text-muted">
                            已选 {visibleSelectedIds.length} 项
                          </span>
              ...
              <div className="space-y-4">
                <div className="bg-white border border-border rounded-sm overflow-hidden">
                  ...
                  <div className="p-5 space-y-3">
                    ...
                    >
                      <option value="">
                        {ordersLoading ? "销售单加载中…" : "请选择待收款销售单"}
                      </option>
                      {eligibleOrders.map((order) => (
                        <option key={order.id} value={order.id}>
                          {order.order_number || `SO-${order.id}`} ·{" "}
                          {order.customer_name || "散客"} · ¥
                          {Number(order.amount_due_cny).toLocaleString()} ·{" "}
                ...
                {selectedPaymentOrder && (
                  <div className="bg-white border border-border rounded-sm overflow-hidden">
                    ...
                    <div className="p-5 space-y-3 text-sm">
                      {(() => {
                        const summary =
                          paymentOrderSummary(selectedPaymentOrder);
                        return (
                          <>
                            <div className="grid gap-2 sm:grid-cols-3">
                              ...
                              <div>
                                <span className="text-xs text-muted">客户</span>
                ...
                <div className="bg-white border border-border rounded-sm overflow-hidden">
                  ...
                  <div className="p-5 space-y-4">
                    <select
                      data-guide="privnote-payment-method"
                      required
                    ...
                    <label className="block text-xs text-muted uppercase tracking-wider">
                      备注
                      <textarea
                        data-guide="privnote-payment-remark"
                        value={remark}
                        onChange={(e) => setRemark(e.target.value)}
                        rows={3}
                        placeholder="备注信息…"
                        className="mt-2 w-full px-3 py-2 border border-border rounded-sm text-sm text-fg bg-white focus:outline-none focus:border-accent resize-none"
                      />
                    </label>
                    <div>
                      <label className="block text-xs text-muted uppercase tracking-wider mb-2">
                ...
                <div className="bg-white border border-border rounded-sm overflow-hidden">
                  ...
                  >
                    ...
                    <span className="text-xs text-accent">
                      {showAttachments ? "收起" : "展开添加"}
                    </span>
                  </button>
                  {showAttachments && (
                    <div className="px-5 pb-5 space-y-4">
                      <p className="text-xs text-muted">
                        可添加外部资源链接（如网盘、文档、视频链接等），客户可在查看页直接打开
                      </p>
