  ...
  useEffect(() => {
    ...
    if (displayData?.title) {
      setMeta({
        ...
        breadcrumbs: [{ label: "首页", to: "/" }, { label: displayData.title }],
      });
    }
      ...
      <div className="min-h-screen flex items-center justify-center bg-cream px-4">
        <div className="bg-white border border-border rounded-sm p-10 text-center w-full max-w-md">
          ...
          <form onSubmit={handlePasswordSubmit} className="space-y-4">
            ...
            />
            {passwordError && (
              <div className="text-sm text-accent">{passwordError}</div>
            )}
            <button
              type="submit"
  ...
  return (
    <div className="min-h-screen bg-cream text-fg">
      ...
      <div className="max-w-5xl mx-auto px-4 pb-10">
        ...
        {/* INVENTORY VIEW */}
        {mode === "inventory" && noteData && (
          <InventoryView data={noteData as InventoryViewData} />
        )}
        ...
        {/* MESSAGE VIEW */}
        {mode === "message" && noteData && (
          <MessageView data={noteData as MessageData} onZoom={setZoomedImage} />
        )}

        {/* QUOTE VIEW */}
        {mode === "quote" && noteData && (
          <QuoteView
            data={noteData as QuoteData}
            createdAt={displayData?.created_at}
            expiresAt={displayData?.expires_at}
              ...
              <tbody>
                {group.items.map((item) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-right font-mono">
                      ¥{item.box_price.toLocaleString()}
                    </td>
                    <td className="px-4 py-3 text-right font-mono">
                      ¥{item.stick_price.toLocaleString()}
                    </td>
                  </tr>
                ))}
              </tbody>
...
function QuoteView({
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white border border-border rounded-sm overflow-hidden hidden md:block">
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="bg-accent-light">
                <th className="px-4 py-3 text-left text-xs text-muted uppercase tracking-wider font-medium w-[36%]">
                  款式
                </th>
                <th className="px-4 py-3 text-left text-xs text-muted uppercase tracking-wider font-medium w-[24%]">
                  中文名
                </th>
                <th className="px-4 py-3 text-center text-xs text-muted uppercase tracking-wider font-medium w-[10%]">
                  支数/盒
                </th>
                <th className="px-4 py-3 text-right text-xs text-muted uppercase tracking-wider font-medium w-[15%]">
                  批发价/盒
                </th>
                <th className="px-4 py-3 text-right text-xs text-muted uppercase tracking-wider font-medium w-[15%]">
                  折算单价/支
                  ...
                  {group.items.map((item) => (
                    ...
                    >
                      <td className="px-4 py-3">
                        <div className="space-y-2">
                          ...
                          <div
                            ...
                            onClick={() =>
                              item.thumb_url && onZoom?.(item.thumb_url)
                            }
                          >
                            {item.thumb_url ? (
                              <img
                                src={item.thumb_url}
                                alt={item.name}
                                className="h-12 inline-block w-auto object-contain p-1"
                              />
                            ) : (
                      ...
                      <td className="px-4 py-3 text-right font-mono text-gold">
                        ¥{item.per_stick_price.toLocaleString()}
                      </td>
                    </tr>
                  ))}
                </>
          ...
          >
            ...
            <div>
              {group.items.map((item) => (
                <div
                  key={`${item.cigar_id}-${item.box_size}`}
                  className="border-b border-border last:border-0 py-3 px-4"
                >
                  <div className="flex justify-between items-start gap-2">
                    <span className="font-medium break-words max-w-[70%]">
                      {item.name}
                    </span>
                    {item.in_stock && (
                  ...
                  <div className="flex justify-between items-baseline mt-2">
                    ...
                    <div className="text-right">
                      ...
                      <span className="text-xs text-gold ml-1.5">
                        ~¥{item.per_stick_price.toLocaleString()}/支
                      </span>
                    </div>
                  </div>
                  {/* Cigar Image */}
      ...
      {/* Footer Note */}
      <div className="bg-white border border-border rounded-sm p-6">
        <div className="text-xs text-muted uppercase tracking-wider font-medium mb-4">
          订货须知
        </div>
      ...
      <div className="text-center py-5 text-muted text-xs border-t border-border">
        CigarDomTabaka · Москва, Молодёжная ул. 3 · +7 929 638-48-78 · WeChat:
        cigardomtabaka
      </div>
    </div>
  );
