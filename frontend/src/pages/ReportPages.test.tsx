...
describe('财务应用月报页面', () => {
  it('从 URL 读取月份并加载月报', async () => {
    ...
    renderPage(<MonthlyBusinessReportPage />, '/accounting/reports/monthly?month=2026-08');
  ...
  it('经营贡献排行保持同一月份并提供返回入口', async () => {
    ...
    renderPage(<ContributionRankingPage />, '/accounting/reports/contributions?month=2026-08');

    expect(await screen.findByText('贡献排行面板')).toBeTruthy();
    expect(api.fetchMonthlyBusinessReport).toHaveBeenCalledWith('2026-08');
    expect(screen.getByRole('link', { name: /返回月度经营报告/ }).getAttribute('href')).toBe('/accounting/reports/monthly?month=2026-08');
  });
});
