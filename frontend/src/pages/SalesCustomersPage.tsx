  ...
  return <div className="w-full animate-fade-in">
    ...
    <section className="grid min-h-[480px] gap-4 lg:grid-cols-[minmax(0,1.35fr)_minmax(340px,.65fr)]">
      <div className="overflow-hidden rounded-md border border-border bg-white shadow-sm">
        <div className="grid grid-cols-[minmax(0,1fr)_minmax(140px,.5fr)_auto] border-b border-border bg-[#FCFBF9] px-4 py-2 text-[10px] font-bold uppercase tracking-wide text-muted"><span>客户 / 最近订单</span><span>备注</span><span>累计有效订单</span></div>
...
function CustomerRow({ customer, selected, onSelect }: { customer: CustomerResult; selected: boolean; onSelect: () => void }) {
  ...
  return <button type="button" onClick={onSelect} className={`grid w-full grid-cols-[minmax(0,1fr)_minmax(140px,.5fr)_auto] gap-4 border-b border-border px-4 py-3 text-left last:border-0 ${selected ? 'bg-[#F5EFE8] shadow-[inset_3px_0_0_#7A1F2E]' : 'hover:bg-[#FCFBF9]'}`}>
    ...
    <span className="min-w-0 self-center truncate text-xs text-muted" title={customer.remark || undefined}>{customer.remark || '—'}</span>
