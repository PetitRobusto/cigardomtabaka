  ...
  return <div className="w-full animate-fade-in">
    ...
    <section className="mb-4 rounded-md border border-border bg-white p-3 shadow-sm">
      <div className="flex flex-wrap items-end gap-2">
        ...
        <label className="text-[11px] font-semibold text-muted">履约<select value={fulfillment} onChange={event => setFulfillment(event.target.value)} className="mt-1 block rounded border border-border bg-white px-2 py-1.5 text-xs text-fg"><option value="">全部履约</option><option value="draft">草稿</option><option value="confirmed">已确认</option><option value="shipped">已出库</option><option value="cancelled">已取消</option><option value="returned">已退货</option></select></label>
