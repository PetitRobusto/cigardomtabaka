...
export default function SalesPage() {
  ...
  const retryAccounts = () => { queryClient.invalidateQueries({ queryKey: ['accounting-accounts'] }); };
  ...
  return <div className="w-full animate-fade-in">
    ...
    {accountsQuery.error && <div className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700"><span>资金账户加载失败，收款和人肉成本动作暂不可用。</span><button type="button" onClick={retryAccounts} className="rounded border border-red-300 px-3 py-1.5 text-xs font-semibold hover:bg-white">重试账户加载</button></div>}
    <div className="grid items-start gap-6 xl:grid-cols-[minmax(440px,.9fr)_minmax(560px,1.1fr)]"><SalesOrderForm onSubmit={create} busy={creating} error={formError} /><section className="rounded-md border border-border bg-white shadow-sm"><div className="flex items-center justify-between border-b border-border px-5 py-4"><div><h2 className="font-display text-lg font-semibold">销售订单</h2><p className="text-xs text-muted">点击订单查看商品、FIFO 批次与动作。</p></div><span className="text-xs text-muted">{ordersQuery.isLoading ? '加载中…' : `${filteredOrders.length} 笔`}</span></div>{ordersQuery.isLoading ? <p className="px-5 py-12 text-center text-sm text-muted">加载订单…</p> : filteredOrders.length === 0 ? <p className="px-5 py-12 text-center text-sm text-muted">暂无符合条件的销售单</p> : filteredOrders.map((order: SalesOrder) => <SalesOrderCard key={order.id} order={order} accounts={accountsQuery.data || []} accountsError={accountsQuery.error ? apiErrorMessage(accountsQuery.error, '资金账户加载失败') : ''} onChanged={refresh} />)}</section></div>
  </div>;
}
